...
Ngrok is a tool that allows Kubefirst to expose a local server to the internet via a tunnel. Kubefirst opens the tunnel during the installation, and Atlantis receive the events via the tunnel. When the installation finish, the terminal window hangs at the handoff screen. The tunnel will be closed once the handoff screen is closed, and the Kubefirst installation process dies.
