...
func AddK3DSecrets(dryrun bool) error {
	...
	dataArgoCiSecrets := map[string][]byte{
		...
		"PERSONAL_ACCESS_TOKEN": []byte(token),
		"username":              []byte(viper.GetString("github.user")),
		"password":              []byte(token),
	}
	...
	usernamePasswordString := fmt.Sprintf("%s:%s", viper.GetString("github.user"), token)
	...
	dataAtlantis := map[string][]byte{
		...
		"TF_VAR_github_token":                 []byte(token),
