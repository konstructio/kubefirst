...
func CreateSoftServe(dryRun bool, kubeconfigPath string) {
	...
	if !viper.GetBool("create.softserve.create") {
		log.Info().Msg("creating soft-serve")
		if dryRun {
			log.Info().Msg("[#99] Dry-run mode, createSoftServe skipped.")
			return
		}
		...
		if errSoftServeApply != nil {
			log.Panic().Msgf("error: failed to apply soft-serve to the cluster %s", errSoftServeApply)
		}
		...
		log.Info().Msg("Skipping: createSoftServe")
	}
...
func ConfigureSoftServeAndPush(dryRun bool) error {
	...
	if dryRun {
		log.Info().Msg("[#99] Dry-run mode, configureSoftserveAndPush skipped.")
		return nil
	}
	...
	if configureAndPushFlag {
		log.Info().Msg("Skipping: configureSoftserveAndPush")
		return nil
	}

	log.Info().Msg("Executing configureSoftserveAndPush")

	totalAttempts := 5
	for i := 0; i < totalAttempts; i++ {

		log.Info().Msgf("Configuring SoftServe, attempt (%d of %d)", i+1, totalAttempts)

		err := configureSoftServe()
		if err != nil {
			log.Warn().Msgf("something went wrong, error is: %v, going to try again...", err)
		...
		log.Info().Msg("waiting SoftServe to finish configuration, sleeping...")
		time.Sleep(30 * time.Second)

		log.Info().Msg("SoftServe successfully configured")
		return nil
	}
...
func configureSoftServe() error {
	...
	log.Info().Msgf("gitClient clone %s %s", url, directory)
	...
	log.Info().Msgf("re-wrote config.yaml at %s/config folder", config.K1FolderPath)
