...
func DetokenizeDirectoryCivoGithubGitops(path string, fi os.FileInfo, err error) error {
	...
	if matched {
		...
		gitopsGitURL := viper.GetString("github.repos.gitops.git-url")
		domainName := viper.GetString("flags.domain-name")
		atlantisWebhookURL := viper.GetString("components.atlantis.webhook.url")
		...
		newContents = strings.Replace(newContents, "<ALERTS_EMAIL>", alertsEmail, -1)
		...
		newContents = strings.Replace(newContents, "<ATLANTIS_ALLOW_LIST>", atlantisAllowList, -1)
		...
		newContents = strings.Replace(newContents, "<KUBE_CONFIG_PATH>", kubeconfigPath, -1)
		...
		newContents = strings.Replace(newContents, "<KUBEFIRST_STATE_STORE_BUCKET>", viper.GetString("kubefirst.state-store.name"), -1)
