...
func DownloadTools(awsConfig *AwsConfig) error {
	...
	go func() {
		...
		helmDownloadTarGzPath := fmt.Sprintf("%s/helm.tar.gz", awsConfig.ToolsDir)
		...
		downloadManager.ExtractFileFromTarGz(
			helmTarDownload,
			fmt.Sprintf("%s-%s/helm", pkg.LocalhostOS, pkg.LocalhostARCH),
			awsConfig.HelmClient,
		)
		err = os.Chmod(awsConfig.HelmClient, 0755)
		if err != nil {
			errorChannel <- err
			return
		}
