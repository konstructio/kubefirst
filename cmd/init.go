...
import (
	...
	"github.com/kubefirst/pkg/flare"
	gitlabSsh "github.com/kubefirst/pkg/ssh"
...
const tracker_stage_0 = "1 - Load properties"
const tracker_stage_1 = "2 - Set .flare initals values"
const tracker_stage_2 = "3 - Test Domain Liveness"
const tracker_stage_3 = "4 - Create SSH Key Pair"
const tracker_stage_4 = "5 - Load Templates"
const tracker_stage_5 = "6 - Download Tools"
const tracker_stage_6 = "7 - Get Account Info"
const tracker_stage_7 = "8 - Create Buckets"
const tracker_stage_8 = "9 - Detokenize"
const tracker_stage_9 = "10 - Send Telemetry"
...
to quickly create a Cobra application.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		Trackers[tracker_stage_0] = &flare.ActionTracker{flare.CreateTracker(tracker_stage_0, int64(1))}
		Trackers[tracker_stage_1] = &flare.ActionTracker{flare.CreateTracker(tracker_stage_1, int64(1))}
		Trackers[tracker_stage_2] = &flare.ActionTracker{flare.CreateTracker(tracker_stage_2, int64(1))}
		Trackers[tracker_stage_3] = &flare.ActionTracker{flare.CreateTracker(tracker_stage_3, int64(1))}
		Trackers[tracker_stage_4] = &flare.ActionTracker{flare.CreateTracker(tracker_stage_4, int64(1))}
		Trackers[tracker_stage_5] = &flare.ActionTracker{flare.CreateTracker(tracker_stage_5, int64(3))}
		Trackers[tracker_stage_6] = &flare.ActionTracker{flare.CreateTracker(tracker_stage_6, int64(1))}
		Trackers[tracker_stage_7] = &flare.ActionTracker{flare.CreateTracker(tracker_stage_7, int64(4))}
		Trackers[tracker_stage_8] = &flare.ActionTracker{flare.CreateTracker(tracker_stage_8, int64(1))}
		Trackers[tracker_stage_9] = &flare.ActionTracker{flare.CreateTracker(tracker_stage_9, int64(1))}
		log.Printf("%s", Trackers)
		...
		Trackers[tracker_stage_0].Tracker.Increment(int64(1))
		Trackers[tracker_stage_1].Tracker.Increment(int64(1))
		...
		Trackers[tracker_stage_2].Tracker.Increment(int64(1))
		...
		Trackers[tracker_stage_3].Tracker.Increment(int64(1))

		log.Println("calling cloneGitOpsRepo() function\n")
		cloneGitOpsRepo()
		log.Println("cloneGitOpsRepo() complete\n\n")
		Trackers[tracker_stage_4].Tracker.Increment(int64(1))
		...
		Trackers[tracker_stage_6].Tracker.Increment(int64(1))
		...
		Trackers[tracker_stage_8].Tracker.Increment(int64(1))
		...
		viper.WriteConfig()
		Trackers[tracker_stage_9].Tracker.Increment(int64(1))
...
func bucketRand() {
	...
	for _, bucket := range buckets {
		...
		log.Println(fmt.Sprintf("bucket %s exists", viper.GetString(fmt.Sprintf("bucket.%s.name", bucket))))
		Trackers[tracker_stage_7].Tracker.Increment(int64(1))
	}
}
...
func getDNSInfo(hostedZoneName string) string {
	...
	for _, zone := range hostedZones.HostedZones {
		if *zone.Name == fmt.Sprintf(`%s%s`, hostedZoneName, ".") {
			...
			log.Println(fmt.Sprintf(`found entry for user submitted domain %s, using hosted zone id %s`, hostedZoneName, zoneId))
			viper.Set("aws.domainname", hostedZoneName)
...
func download() {
	...
	os.Chmod(kubectlClientPath, 755)
	...
	Trackers[tracker_stage_5].Tracker.Increment(int64(1))
	...
	os.Chmod(fmt.Sprintf("%s/terraform", unzipDirectory), 755)
	Trackers[tracker_stage_5].Tracker.Increment(int64(1))
