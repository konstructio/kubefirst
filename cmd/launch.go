...
var (
	...
	// Describes a context-exclusive cluster name generated by the console
	intClusterName string
	// HTTP client
...
func launchListClusters() *cobra.Command {
	launchListClustersCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			dir := fmt.Sprintf("%s/.k1/%s", homeDir, clusterName)
			if _, err := os.Stat(dir); os.IsNotExist(err) {
				log.Infof("unable to list clusters %s - cluster %s directory does not exist", clusterName, dir)
			}

			// Port forward to API
			kubeconfigPath := fmt.Sprintf("%s/.k1/%s/kubeconfig", homeDir, clusterName)
			if _, err := os.Stat(dir); os.IsNotExist(err) {
				log.Infof("unable to list clusters %s - kubeconfig file does not exist", clusterName)
			}
...
func launchDeleteCluster() *cobra.Command {
	launchDeleteClusterCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			dir := fmt.Sprintf("%s/.k1/%s", homeDir, clusterName)
			if _, err := os.Stat(dir); os.IsNotExist(err) {
				log.Infof("unable to delete cluster %s - cluster %s directory does not exist", clusterName, dir)
			}

			// Port forward to API
			kubeconfigPath := fmt.Sprintf("%s/.k1/%s/kubeconfig", homeDir, clusterName)
			if _, err := os.Stat(dir); os.IsNotExist(err) {
				log.Infof("unable to delete cluster %s - kubeconfig file does not exist", clusterName)
			}
			...
			// Delete cluster
			req, err := http.NewRequest(http.MethodDelete, fmt.Sprintf("http://localhost:%v/api/v1/cluster/%s", randPort, intClusterName), nil)
			if err != nil {
				log.Fatalf("error creating request to api: %s", err)
			}
			req.Header.Set("Content-Type", "application/json")
			...
			fmt.Printf("Submitted request to delete cluster %s: %s - follow progress with `kubefirst console list-clusters`", intClusterName, objMap["message"])
		},
	}

	launchDeleteClusterCmd.Flags().StringVar(&intClusterName, "name", "", "name of the cluster to delete (required)")
	launchDeleteClusterCmd.MarkFlagRequired("name")
